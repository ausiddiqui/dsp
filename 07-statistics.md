...
>> P(Twin) = P(Fraternal) + P(Identical) = 0.011333
...
>>
>> print(p_f, p_i, p_t)
>> # 0.008 0.00333333333333 0.0113333333333
...
>> In Frequentist statistics the data is considered to be a random sample from a repeatable experiment where the parameters have fixed values. In comparison Bayesian statistics considers the data to be fixed and the parameters being unknown and being inferred probabilistically. If X is an observed event in a study / experiment with X ~ distribution(n, p) where n is the number of observations and p is the probability of X. The Frequentist interpretation will be that the parameter p is constant and the experiment is repeatable. Whereas in the Bayesian method, the p can vary but the data in this case would be fixed.
